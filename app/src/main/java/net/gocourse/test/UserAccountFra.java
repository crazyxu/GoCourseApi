...
public class UserAccountFra extends Fragment implements View.OnClickListener{
    //功能按钮
    ...
    class MyThread extends Thread{
        ...
        public void run() {
            switch (code){
                //登录
                case ACTION_LOGIN:
                    resEntity = new UserAccountAction().userLogin((String)params.get("userName"),(String)params.get("userPwd"));
                ...
                case ACTION_REGISTER:
                    resEntity = new UserAccountAction().userReg((String) params.get("userName"), (String) params.get("userPwd"));
                    myHandler.sendEmptyMessage(ACTION_REGISTER);
                    break;
                //注销
                case ACTION_LOGOUT:
                    resEntity = new UserAccountAction().userLogout((String)params.get("token"));
                    app.setToken("");
                    app.setExpire(0);
                ...
                case ACTION_CHANG_PWD:
                    resEntity = new UserAccountAction().changPwd((String) params.get("token"),(String) params.get("oldPwd"), (String) params.get("newPwd"));
                    //如果登录成功，将token和expire保存到Application中作为全局量
                    if (resEntity.isResState()) {
                        app.setToken((String)resEntity.getResData().get("token"));
                        app.setExpire((Integer) resEntity.getResData().get("expire"));
